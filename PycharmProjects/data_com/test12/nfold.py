...
for index ,(train_index,test_index) in enumerate(skf.split(X , y)): #训练数据五折
    ...
    pickle.dump(model_xgboost, open("model_xgboost{}.pickle.dat".format(cnt), "wb"))
    gc.collect()  # 垃圾清理，内存清理

    y_pred_test = model_xgboost.predict(test_x)
...
result_append.to_csv('result_1208.txt', header=False, index=False, sep='\t')
